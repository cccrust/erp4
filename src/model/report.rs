...
pub fn sales_report(conn: &Connection, from: Option<&str>, to: Option<&str>) -> Result<Vec<SalesRow>> {
    let mut sql = "SELECT oi.product_id, p.name, SUM(oi.quantity), SUM(oi.quantity * oi.unit_price)
        FROM order_items oi
        JOIN orders o ON o.id = oi.order_id
        JOIN products p ON p.id = oi.product_id
        WHERE o.status IN ('confirmed', 'shipped', 'delivered')".to_string();
...
pub fn inventory_report(conn: &Connection) -> Result<Vec<product::Product>> {
    product::list_products(conn, None, None)
}
